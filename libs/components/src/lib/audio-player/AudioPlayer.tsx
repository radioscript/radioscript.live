...
import { MusicPlaylist, Next, Pause, Play, Previous, Repeat, RepeateMusic, RepeateOne, TimerPause } from 'iconsax-react';
  ...
  return (
    <div className="fixed bottom-0 start-0 end-0 bg-background border-t shadow-md p-4 flex items-center z-50">
      <div className="app-container flex items-center gap-4 w-full">
        ...
        <div className="text-lg font-semibold truncate w-64 hidden md:inline-block">{currentTrack.title}</div>
        <div className="flex gap-2">
          <Button onPress={playNext} color="secondary" variant="light" isIconOnly>
            <Next size="24" className="stroke-secondary" />
          </Button>
          <Button onPress={togglePlay} color="secondary" variant="light" isIconOnly>
            {isPlaying ? <Pause size="24" className="stroke-secondary" /> : <Play size="24" className="stroke-secondary" />}
          </Button>
          <Button onPress={playPrev} color="secondary" variant="light" isIconOnly>
            <Previous size="24" className="stroke-secondary" />
          </Button>
        </div>
        ...
        <div className="flex gap-2">
          <Button color="secondary" variant="light" isIconOnly>
            <MusicPlaylist size="24" className="stroke-secondary" />
          </Button>
          <Button onPress={cycleRepeat} color="secondary" variant="light" isIconOnly>
            {repeatMode === 'all' && <RepeateMusic className="stroke-secondary" size="24" />}
            {repeatMode === 'none' && <Repeat className="stroke-secondary" size="24" />}
            {repeatMode === 'one' && <RepeateOne size="24" className="stroke-secondary" />}
          </Button>

          <Button color="secondary" variant="light" isIconOnly>
            <TimerPause size="24" className="stroke-secondary" />
          </Button>
        </div>
      </div>
    </div>
